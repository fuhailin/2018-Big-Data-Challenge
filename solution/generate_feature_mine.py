...
def get_train_label(feat_path, label_path):
    ...
    feat_create = pd.read_csv(feat_path + '/create.csv', usecols=['user_id'])
    feat_activity = pd.read_csv(feat_path + '/activity.csv', usecols=['user_id'])
    feat_data_id = np.unique(pd.concat([feat_register, feat_launch, feat_create, feat_activity]))
    ...
    label_create = pd.read_csv(label_path + '/create.csv', usecols=['user_id'])
...
def get_create_feature(video):
    ...
    video_last_day = video.groupby(['user_id'])['create_day'].max().rename('video_last_day').reset_index()
    video_rest_day = (video.groupby(['user_id'])['create_day'].max() - end_day).rename('video_rest_day').reset_index()
...
def deal_feature(path, user_id):
    ...
    create = pd.read_csv(path + '/create.csv')
