...
def predict_func(gbm):
    ...
    df_result.to_csv('result/lgb_result_lgb_tune.csv', index=False)
...
if __name__ == '__main__':
    ...
    # params = (boosting_type='gbdt', objective= 'binary', metric= 'binary_error', learning_rate = 0.01)
    gbm = LGBMClassifier(boosting_type='gbdt', objective='binary', metric='binary_error', learning_rate=0.01)

    # 待调整的超参数  
    parameters = {
        'num_leaves': range(20, 22, 2),
        'max_depth': range(3, 9, 1),
    }
    clf = GridSearchCV(gbm, parameters, n_jobs=8)
    clf.fit(X_train, Y_train)  # 你和模型
    ...
    y_pred_array = clf.predict(X_test)  # 预测结果
    print('f1_score：' + str(sklearn.metrics.f1_score(Y_test, y_pred_array)))
...
#          'learning_rate' : 0.01,
#          }
# gbm = train_func(clf.best_params_)
